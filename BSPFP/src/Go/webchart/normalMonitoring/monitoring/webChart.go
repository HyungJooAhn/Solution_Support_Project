...
func main() {
	...
	stmtInsert, err := sqlContext.Prepare("insert into monitoring (ID, date, time, traffic) values (?,TO_DATE(?, 'YYYY-MM-DD'),?,?)")
	if err != nil {
		log.Fatal(err)
	}
