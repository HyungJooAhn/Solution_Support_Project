	...
	src.Map(func(packet string) {
		...
		if packetTotalCount == 1 {
			go doEvery(time.Duration(1)*time.Second, func() {
				...
				// Get Memory State ******
				memoryCommand := exec.Command("/bin/sh", "-c", "free -m | grep Mem | awk {'print$3'}")
				memoryOutput, _ := memoryCommand.Output()
				...
				fmt.Println("=========================================")
				fmt.Println("Date :", nowDate, nowTime)
				fmt.Printf("Term(X) : %s%d%s\n", CLR_G, int(timeQueue.Bottom()), CLR_N)
				fmt.Printf("Traffic(Y) : %s%3f%s MB/s\n", CLR_G, packetTotalSizeMB, CLR_N)
				fmt.Println("Max Traffic Date :", maxDate, maxTime)
				fmt.Printf("Max Traffic : %s%3f%s MB/s\n", CLR_B, maxTraffic, CLR_N)
				fmt.Println("=========================================")
		...
		if attackCount == 1 {
			go doEvery(time.Duration(1)*time.Second, func() {
				...
				if strings.Contains(*attackName, "TCP") {
					tcpCh <- *attackByte
					fmt.Printf("%s %v Detection : %.0fBPS, %dpps %s\n", CLR_R, *attackName, *attackByte, *attackCountNumber, CLR_N)
