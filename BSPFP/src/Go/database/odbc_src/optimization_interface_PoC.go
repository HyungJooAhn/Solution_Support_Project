...
import (
	"flag"
	_ "keti/datastruct/enzyme/optimizer"
	context_sql "keti/squall/sql/context_sql"
	"log"
	_ "odbc/driver"
)
...
func main() {
	...
	sqlContext := context_sql.SQLContext()
	sqlContext.Open("odbc", "DSN=Ketiodbc")

	// Argument Configure

	count := flag.Int("count", 100, "Select Count")
	opt := flag.Bool("opt", false, "Optimizer")

	flag.Parse()

	// Query and Optimizer

	query := "SELECT NAME FROM (SELECT ID, NAME FROM person WHERE NAME='Henry') P WHERE P.ID=1"
	if *opt == true {
		query = sqlContext.QueryOptimizer(query)
