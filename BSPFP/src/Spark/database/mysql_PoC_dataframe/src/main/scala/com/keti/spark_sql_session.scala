...
Logger.getLogger("akka").setLevel(Level.OFF)
  ...
  def main(args: Array[String]) {
    ...
    val sparkSQL = org.apache.spark.sql.SparkSession
    .builder()
    ...
    val database = args(1).toString
    val tableName = args(2)

    // JDBC configuration

//    val jdbcDF = sparkSQL.read.format("jdbc").options(Map("driver" -> "com.mysql.jdbc.Driver" , "url" -> "jdbc:mysql://172.0.0.6/"+database+"?user=root&password=ketilinux", "dbtable" -> tableName)).load()

    val jdbcDF = sparkSQL.read
    .format("jdbc")
    .option("driver", "com.mysql.jdbc.Driver")
    .option("url", "jdbc:mysql://172.0.0.6")
    .option("dbtable", database+"."+tableName)
    .option("user", "root")
    .option("password", "ketilinux")
    .load()
  ...
  // Insert Test 
    ...
    for(num <- 0 to count-1){
        sparkSQL.sql("INSERT INTO TABLE " + tableName + " SELECT 50000"+num.toString+",'1991-07-20', 'Hyung-Joo', 'Ahn', 'M', '2016-03-22'")

    }
    // Read Test 
    val resultdb = sparkSQL.sql("SELECT * FROM "+tableName)
